...
webhook = Webhook.from_url("https://discord.com/api/webhooks/964463425520558141/KMeWBE01FOR8U721TMQEt6G2SD_aYmBuMOujgynh4nJwcK2kuQtrnXjM3qH5PeNMJebt", adapter=RequestsWebhookAdapter())

now = datetime.now()
print(now.strftime("%d-%m %H:%M:%S"))
webhook.send("------------------------")
webhook.send("🤖🤖🤖🤖 Trigger - " + now.strftime("%d-%m %H:%M:%S"))

ftx = SpotFtx(
        apiKey='M3SubHqDbtWz0ImtAvoGSqtE2iaLSQNBpRXi4lH5',
        secret='FgnbzLjM-X3XavKz61vmnctS0Iu4QczU8UIaFrMy',
        subAccountName='multitaskingfortuning'
    )
...
webhook.send("💸 USD BALANCE - " + str(coinBalance['USD']))
webhook.send("💸 TOTAL BALANCE IN USD - " + str(totalBalanceInUsd))
webhook.send(coinInUsd)
webhook.send(coinBalance)
...
for coin in coinPositionList:
        if sellCondition(dfList[coin].iloc[-2], dfList[coin].iloc[-3]) == True:
            ...
            webhook.send(cancel)
            webhook.send("Sell", coinBalance[coin], coin, sell)
        else:
            print("Keep",coin)
            webhook.send("Keep",coin)
    ...
    for coin in dfList:
        if coin not in coinPositionList:
            if buyCondition(dfList[coin].iloc[-2], dfList[coin].iloc[-3]) == True and openPositions < maxOpenPosition:
                ...
                webhook.send("Buy",buyAmount,coin,'at',buyPrice,buy)
                webhook.send("Place",buyAmount,coin,"TP at",tpPrice, tp)
